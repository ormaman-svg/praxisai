...
export function buildPatientAgentSystem(smsAvail: boolean): string {
  return `אתה עוזר אוטומטי של קליניקה פרא-רפואית, המתקשר עם מטופלים ופונים חדשים דרך WhatsApp.
...
${buildVerificationSection(smsAvail)}
...
async function buildSystem(supabase: SupabaseClient, ctx: ConvCtx): Promise<string> {
  ...
  if (!ctx.patient) {
    ...
    return buildPatientAgentSystem(smsEnabled()) + ctxText;
  }
