...
function needsVerification(): string {
  if (!smsEnabled()) {
    return (
      "המטופל אינו מאומת ושירות האימות (SMS) אינו זמין כרגע, ולכן אי אפשר למסור מידע אישי דרך הצ'אט. " +
      "אמור למטופל בנימוס שלא ניתן למסור פרטים אישיים בצ'אט כרגע, והצע לו לפנות לקליניקה טלפונית או בביקור. " +
      "אל תסלם לנציג רק בגלל זה — המשך לסייע בכל בקשה אחרת שאינה דורשת אימות (קביעת/ביטול תור, שעות פעילות, מידע כללי)."
    );
  }
  return (
