    ...
    <div ref={ref} className="relative">
      ...
      >
        ...
        <div className="min-w-0 flex-1">
          ...
          <div className="text-[11px] text-slate-400">{active ? ROLE_HE[active.role] : ""}</div>
        </div>
        {memberships.length > 1 && <ChevronsUpDown size={14} className="shrink-0 text-slate-500" />}
      </button>
        ...
        <div className="absolute inset-x-0 top-full z-30 mt-1.5 overflow-hidden rounded-lg border border-navy-700 bg-navy-800 shadow-pop">
          {memberships.map((m) => (
            <button
              ...
              className="flex w-full items-center gap-2.5 px-3 py-2.5 text-start transition-colors hover:bg-navy-700 disabled:opacity-50"
            >
              <div className="min-w-0 flex-1">
                <div className="truncate text-[13px] font-medium text-white">{m.clinics?.name}</div>
                <div className="text-[11px] text-slate-400">{ROLE_HE[m.role]}</div>
              </div>
              {m.clinic_id === activeClinicId && <Check size={14} className="text-brand" />}
            </button>
          ))}
        </div>
      )}
